...
//				layer.confirm('确认顾客已收到货了吗?', {icon: 3, title:'提示'}, function(index){
				layer.confirm('确认订单价格无误吗?', {icon: 3, title:'提示'}, function(index){
...
//		if(this.isCod && (this.orderStatus == OrderStatus.ORDER_NOT_PAY)){
		if(this.orderStatus == OrderStatus.ORDER_NOT_PAY){
...
//			(!this.isCod && this.payStatus == OrderStatus.PAY_NO)// 非货到付款的话，是未支付时使按钮可用
			(!this.isCod && this.orderStatus == OrderStatus.ORDER_CONFIRM)// 非货到付款，订单确认，按钮可用
...
//		}
		// 完成按钮
		if (this.shipStatus == OrderStatus.SHIP_NO && this.orderStatus==OrderStatus.PAY_YES) {
			$("#rog").removeClass("layui-btn-disabled");
		}
//		// 订单退货
