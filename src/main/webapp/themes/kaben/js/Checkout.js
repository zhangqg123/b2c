...
//		var is_print = $("#isPrint").val();
		...
		if($("#isPrint").val()>2||$("input[name='bonusid']").val()==1||$("input[name='bonusid']").val()==2){
			var html = $(".add-success-msg").html();
			$.dialog({ title:'提示信息',content:html,lock:true,init:function(){
				var self = this;
				var code ;
				//$(".ui_content .btn").jbtn();
				$(".ui_content .confirm_btn").click(function(){
					code= $(".ui_content .message input").val();
					if(code!=""){
						self.close();     //关闭自己
						$("#code").val(code);
				...
				$(".ui_content .cancel_btn").click(function(){
					self.close();     //关闭自己
				});
