var Cart = {
    ...
    cartNumChanged: function (_this) {
        ...
        if(productid=="-1"){
            self.updatePeoNum(itemid, _val, _this);        	
        }else{
        	if(productid=="-2"){
        		if(_val=="z" || _val=="Z"){
        			self.updatePrice(itemid, 0, _this)
        		}else{
...
//				});
    ...
    updatePrice: function (itemid, price, price_input) {
        var self = this;
        $.ajax({
            url     : ctx + "/api/shop/cart/update-price.do",
            data    : "cartid=" + itemid + "&price=" + price,
            dataType: "json",
            success : function (result) {
                if (result.result == 1) {
                    if (result.message == "success") {
                        self.refreshTotal();
                        price1     = self.changeTwoDecimal_f(price * 1);
                        $("tr[itemid=" + itemid + "] .itemTotal").html("￥" + price1);
                        $("tr[itemid=" + itemid + "] .cart-quantity").html("1");
                        $("tr[itemid=" + itemid + "] .cart-price").html(price);
//                      price_input.val(price);
    ...
    updatePeoNum: function (itemid, num, num_input) {
        var self = this;
//        alert("itemid:"+itemid+","+num+","+num_input)
        $.ajax({
            url     : ctx + "/api/shop/member/update-num.do",
            data    : "memberid=" + itemid + "&num=" + num ,
            dataType: "json",
            success : function (result) {
                if (result.result == 1) {
                    num_input.val(num);
